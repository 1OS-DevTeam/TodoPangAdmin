...
import { DeployReview, Review } from "src/types/review";
...
const StatusChangeModal = ({ isOpen, onClose, selectedData }: Props) => {
  ...
  const { mutate: updateCategories } = useMutation({
    mutationFn: (reviews: DeployReview[]) => {
      setIsAddLoading(true);

      return services.Reveiw.deployReview(reviews);
    },
    onSuccess: async () => {
  ...
  const handleSubmit = () => {
    const updatedList: DeployReview[] = [];
  ...
  return (
    ...
    >
      <div className="mb-40">
        <p className="mb-10 text-gray-6 tracking-tight">선택한 리뷰</p>
          ...
          >
            <div className="flex items-center">
              ...
              <p className="tracking-tight font-normal mr-10 text-14">
                {selected.title} (ID: {selected.reviewId})
              </p>
            </div>
            <p
              className={`${selected.status === 1 ? "bg-yellow-0 text-yellow-6" : selected.status === 2 ? "bg-blue-0 text-blue-6" : "bg-gray-1 text-gray-6"} inline-block px-6 py-2 rounded text-14`}
            >
              {
                Constants.CATEGORY_STATUS[
                  selected.status as keyof typeof Constants.CATEGORY_STATUS
                ]
              }
            </p>
          </div>
